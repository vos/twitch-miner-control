...
def _campaign_details(cid="c1", name="Campaign One", drops=(), game=True,
                      allow=("100", "200")):
...
def _drop_details(did="d1", minutes=60, subs=0, benefits=("Crate", "Crate")):
    """A drop as the *details* query reports it -- no self_edge, since that
    only exists on the inventory's in-progress shape."""
    return SimpleNamespace(
        id=did,
        name="Crate",
        benefits=list(benefits),
...
def _campaigns_handler(dashboard=(), details=None):
    h = handler(balances={"alpha": 10}, live={"42": True})
    h.session.gql.get_viewer_drops_dashboard = lambda: SimpleNamespace(
        campaigns=list(dashboard)
    )
    resolved = details if details is not None else {}
    h.session.gql.get_drop_campaign_details = lambda ids: [
        SimpleNamespace(campaign=resolved[i]) for i in ids if i in resolved
    ]
    return h


def test_campaigns_flattens_active_campaigns():
    h = _campaigns_handler(
        dashboard=[SimpleNamespace(id="c1", status="ACTIVE")],
        details={"c1": _campaign_details(drops=[_drop_details()])},
    )
    out = h.handle({"id": 1, "op": "campaigns"})
    assert out["ok"] is True
    ...
    # Deduped: Twitch lists one edge per benefit instance, so a drop
    # granting two of an item repeats the name and "Crate, Crate" would
    # read as a bug.
    assert campaign["drops"][0]["benefits"] == ["Crate"]
    # Epoch ms, not datetime -- json.dumps in serve() would raise on a
    # datetime, the same trap _next_drop documents for endsAt.
    assert campaign["startsAt"] == 1788264000000  # 2026-09-01 12:00 UTC
    assert isinstance(campaign["endsAt"], int)


def test_campaigns_requests_only_active_ids():
    h = _campaigns_handler(
        dashboard=[
            SimpleNamespace(id="c1", status="EXPIRED"),
            SimpleNamespace(id="c2", status="ACTIVE"),
        ],
        details={"c1": _campaign_details(cid="c1"),
                 "c2": _campaign_details(cid="c2")},
    )
    asked = []
    inner = h.session.gql.get_drop_campaign_details
    h.session.gql.get_drop_campaign_details = lambda ids: (
        asked.extend(ids) or inner(ids)
    )
    out = h.handle({"id": 1, "op": "campaigns"})
    # The expired campaign is never even asked about: details are the
    # expensive half and there can be 100+ campaigns.
    assert asked == ["c2"]
    assert [c["id"] for c in out["data"]["campaigns"]] == ["c2"]


def test_campaigns_is_empty_when_nothing_is_active():
    h = _campaigns_handler(
        dashboard=[SimpleNamespace(id="c1", status="EXPIRED")],
    )
    out = h.handle({"id": 1, "op": "campaigns"})
    assert out["data"]["campaigns"] == []


def test_campaigns_tolerates_a_dashboard_with_no_campaigns():
    h = handler(balances={"alpha": 10}, live={"42": True})
    h.session.gql.get_viewer_drops_dashboard = lambda: SimpleNamespace(
        campaigns=None
    )
...
def test_campaigns_survives_missing_fields():
    """A miner build whose parser predates a field must degrade to a
    missing value rather than failing the whole batch."""
    ...
    assert campaign["endsAt"] is None
    assert campaign["allowChannelIds"] == []
    assert campaign["drops"] == []


def test_campaigns_survives_a_drop_without_benefits():
    h = _campaigns_handler(
        dashboard=[SimpleNamespace(id="c1", status="ACTIVE")],
        details={"c1": _campaign_details(drops=[
            SimpleNamespace(id="d1", name="Crate",
                            required_minutes_watched=60, required_subs=0),
        ])},
    )
    out = h.handle({"id": 1, "op": "campaigns"})
